const CACHE_NAME = 'instareport-v3-8';

// Static app-shell assets only. NOTE: cdn.tailwindcss.com removed — index.html
// v3.8+ embeds a compiled static CSS bundle directly, no external Tailwind
...
self.addEventListener('install', (e) => {
  e.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      console.log('Service Worker: Caching app shell (v3.8)');
      return Promise.all(
        APP_SHELL.map(url =>
...
// FIX (v3.8): these must always go to the network. Previously everything
