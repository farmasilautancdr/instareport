const CACHE_NAME = 'instareport-v4-0';

// Static app-shell assets only. NOTE: cdn.tailwindcss.com removed — index.html
// v4.0+ embeds a compiled static CSS bundle directly, no external Tailwind
...
self.addEventListener('install', (e) => {
  e.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      console.log('Service Worker: Caching app shell (v4.0)');
      return Promise.all(
        APP_SHELL.map(url =>
...
// FIX (v4.0): these must always go to the network. Previously everything
